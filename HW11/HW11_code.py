...
red = np.linspace(1,1,cam.width * 3, dtype=np.float)
green = np.linspace(0,0,cam.width * 3, dtype=np.float)
blue = np.linspace(0,0,cam.width * 3, dtype=np.float)
ind = 0
...
while True:
    ...
    b=0
    d=0
    for a in range(0,2 * cam.width * 3,4):
        if a == 0:
            b=80
        if d>156:
            b=1116
