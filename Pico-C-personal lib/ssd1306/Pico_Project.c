...
#warning i2c / oled_i2d example requires a board with I2C pins
    ...
    sprintf(message, "kalemaklemkalemkalemkalem");
...
void write(char *message,char line){
    ...
    int col=0;
    while (!(message[col] == 0x00)){col++;}
    
    struct render_area area1 = {start_col: 1, end_col : col*5 , start_page : line, end_page : line };
    calc_render_area_buflen(&area1);
