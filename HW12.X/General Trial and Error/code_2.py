...
color = np.linspace(0,0,1*cam.height, dtype=np.float)
row_1 = np.linspace(0,0,cam.height, dtype=np.float)
row_sum = np.linspace(0.01,0.01,1,dtype=np.float)
row_mass_sum = np.linspace(0,0,1,dtype=np.float)
row_center = np.linspace(0,0,1,dtype=np.float)
ind = 0
...
while True:
    ...
    ind = 0
    for a in range(1,cam.height/2 * 1 + 1 , 1):

        if a<=15:
            d=2*40*a - 2*38 #getting the 38th column
